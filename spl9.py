...
def launch_smart_grid(pkg, uri, index=0, total=1):
    ...
    # 1. Hitung koordinat target menggunakan logika grid cerdas
    ...
    # 2. Buka aplikasi via Fullscreen (--windowingMode 1) agar pasti tampil & merender di foreground
    print(f"-> Membuka {pkg} dalam mode Fullscreen untuk inisialisasi awal...")
    run_root(f'am start --windowingMode 1 -a android.intent.action.VIEW -d "{uri}" {pkg}')

    # 3. Beri jeda agar aplikasi loading dan masuk ke foreground dengan sempurna (misal 3 detik)
    print("-> Menunggu aplikasi merender tampilan di foreground...")
    time.sleep(3.0)

    # 4. Tangkap Task ID, lalu ubah dan sesuaikan ukurannya ke koordinat grid cerdas
    task_id = get_task_id(pkg)
    if task_id:
        print(f"-> Task ID ditemukan ({task_id}), menerapkan ukuran grid & fokus layar...")
        run_root(f"am task resize {task_id} {left} {top} {right} {bottom}")
        run_root(f"am task switch {task_id}")
        print("Berhasil! Jendela aplikasi otomatis masuk ke posisi grid layar.")
    else:
...
if __name__ == "__main__":
    ...
    # Sesuaikan parameter index dan total jika ingin menjalankan banyak akun
    launch_smart_grid(pkg, uri, index=0, total=4)
